...
public class DeploymentServiceImpl implements DeploymentService {
    ...
    public DeploymentResponse initiateDeployment(DeploymentRequest request) {
        ...
        try {
            ...
            sendEvent(savedDeployment.getId().toString(), DeploymentState.INITIAL, DeploymentEvent.AUTHENTICATION_SUCCESS);
            ...
            sendEvent(savedDeployment.getId().toString(), DeploymentState.AUTHENTICATED, DeploymentEvent.DEPLOYMENT_REQUEST_SUCCESS);
    ...
    public DeploymentResponse whitelistContract(String requestId) {
        ...
        try {
            ...
            sendEvent(deployment.getId().toString(), DeploymentState.WHITELIST_REQUESTED, DeploymentEvent.WHITELIST_HASH_FETCHED);
            ...
            sendEvent(deployment.getId().toString(), DeploymentState.WHITELIST_HASH_RETRIEVED, DeploymentEvent.WHITELIST_HASH_SIGNED);
            ...
            sendEvent(deployment.getId().toString(), DeploymentState.WHITELIST_HASH_SIGNED, DeploymentEvent.WHITELIST_APPROVED);
            ...
            if (registered) {
                ...
                sendEvent(deployment.getId().toString(), DeploymentState.WHITELIST_APPROVED, DeploymentEvent.TOKEN_REGISTERED);
                
                // Mark as completed
                sendEvent(deployment.getId().toString(), DeploymentState.TOKEN_REGISTERED, DeploymentEvent.REGISTER_TOKEN);
            }
    ...
    public DeploymentResponse getDeploymentStatus(String requestId) {
        ...
        // If the contract is in DEPLOYMENT_APPROVED state, check status to see if it's deployed
    ...
    private void checkDeploymentStatus(SmartContractDeployment deployment) {
        ...
        if ("deployed".equalsIgnoreCase(status)) {
            ...
            sendEvent(deployment.getId().toString(), DeploymentState.DEPLOYMENT_APPROVED, DeploymentEvent.DEPLOYMENT_COMPLETED);
        }
    }
    ...
    private StateMachine<DeploymentState, DeploymentEvent> build(String deploymentId, DeploymentState currentState) {
        ...
        stateMachine.getStateMachineAccessor()
                .doWithAllRegions(accessor -> {
                    // Using the non-deprecated method for StateMachineContext in Spring State Machine 3.2.x
                    accessor.resetStateMachineReactively(new DefaultStateMachineContext<>(currentState, null, null, null, null))
                        .block();
                });
